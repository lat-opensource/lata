...
class Pattern(General):
    ...
    def output_code(self, i, extracted, outerbits, outermask):
        ...
        if fmt_refs:
            ...
            self.output_fields(ind, lambda n: type_prefix + '_u.f_' + arg + '.' + n)
            assert not extracted, "dangling fmt refs but it was already extracted"
        if not extracted:
            output(ind, self.base.extract_name(),
                   '(ctx, &',type_prefix,'_u.f_', arg, ', insn);\n')
        if not fmt_refs:
            # pattern fields last
            self.output_fields(ind, lambda n: type_prefix + '_u.f_' + arg + '.' + n)
   
        # output(ind, '#ifdef CONFIG_LATA\n')
        output(ind, 'ctx->arg = &',type_prefix,'_u.f_', arg, ';\n')
        output(ind, 'ctx->insn_type = AARCH64_', type_prefix.upper(), '_', self.name, ';\n')
...
class Tree:
    ...
    def output_code(self, i, extracted, outerbits, outermask):
        ...
        if not extracted and self.base and not self.base.dangling_references():
            output(ind, self.base.extract_name(),
                   '(ctx, &',type_prefix,'_u.f_', self.base.base.name, ', insn);\n')
            extracted = True
...
def main():
    ...
    insn_type_num = {s: i for i, s in enumerate(insn_type)}

    #generate type enum
    match = re.search(r'(?<=/)[^/]+?(?=\.)', input_file)
    type_prefix = match.group().replace("-", "_")
    output('typedef enum AARCH64_INSN_TYPE_', type_prefix.upper(),' {\n')
    output(str_indent(4), ' AARCH64_', type_prefix.upper(), '_NULL,\n')
    for t in sorted(insn_type_num):
        output(str_indent(4), ' AARCH64_', type_prefix.upper(), '_', t, ',\n')
    if type_prefix == 'a64':
        output(str_indent(4), ' AARCH64_A64_DATA_PROC_REG,\n')
        output(str_indent(4), ' AARCH64_A64_DATA_PROC_SIMD_FD,\n')
    output('} AARCH64_INSN_TYPE_', match.group().replace("-", "_").upper(), ';\n\n')  

    output('union {\n')
    for n in sorted(arguments.keys()):
        f = arguments[n]
        output(str_indent(4), f.struct_name(), ' f_', f.name, ';\n')
    output('} ',match.group().replace("-", "_"),'_u;\n\n')
